...
const currRingtone = computed(() => userStore.timerObj.chosen_ringtone); // hardcode link
const currVolume = computed(() => userStore.timerObj.ringtone_volume[0]);
const chosenRingtone = 'mindfulringtone'; // To store the chosen ringtone from state
...
const fetchRingtones = async () => {
  try {
    const ringtoneFolderRef = storageRef(storage, "mindful_ringtone/");
...
const setChosenRingtoneUrl = () => {
  const ringtoneFileName = `${chosenRingtone}.mp3`;
